...
var connStr = "host=localhost port=5432 user=postgres password='root' dbname='Smart Notes' sslmode=disable"

//DecodedUserID is used to convert received user_id from json to uuid
type DecodedUserID struct {
	UserID string `json:"user_id"`
}

//Note containts main note data
type Note struct {
	NoteID        uuid.UUID `json:"note_id" sql:",type:uuid"`
...
func getUserNotes(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		var tmp_r Note
		err = rows.Scan(&tmp_r.NoteID, &tmp_r.Name, &tmp_r.Category)
		if err != nil {
			panic(err)
		}
		if tmp_r.Category != "Нет категории" {
			...
			sub_rows, err := db.Query(sqlStatement, userID, tmp_r.Category)
			if err != nil {
				panic(err)
			}
			for sub_rows.Next() {
				err = sub_rows.Scan(&tmp_r.CategoryColor, &tmp_r.TextColor)
				if err != nil {
					panic(err)
				}
			}
		} else {
			tmp_r.CategoryColor = "white"
			tmp_r.TextColor = "black"
		}
